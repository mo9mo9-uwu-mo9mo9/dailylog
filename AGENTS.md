...
PR を作成した作成者は、マージ前に必ず「セルフレビュー」を行い、PR テンプレートの該当欄を埋めるか、PR にコメントで投稿してください（題名例: `セルフレビュー（作成者）`）。観点の例:

- 受け入れ基準: 要件を満たしているか（画面/印刷/API それぞれ）
- UI/印刷: レイアウト崩れがないか、拡大縮小・印刷プレビューでの確認
- セキュリティ: XSS/入力検証/認可（Basic 認証等）の扱い
- エラーハンドリング: 想定外入力・ネットワーク失敗時の挙動
- パフォーマンス: 無駄な再描画/重い処理がないか
- i18n/日本語表記: 文言・単位・日付表示の整合
- アクセシビリティ: タイトル/ツールチップ/対比・可読性
- テスト/CI: 単体テスト/format/lint の通過、手動確認の記録
- 影響範囲: 既存ページ（月次/印刷/API）への影響
- メタ情報: 関連 Issue 記載、ラベル、Conventional Commits、スクショ/ログ

備考: セルフレビューはレビューワの負担軽減・品質向上が目的です。短くてもよいので、差分の意図と確認結果を明記してください。
